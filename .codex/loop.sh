#!/usr/bin/env bash
# Codex non-interactive loop shim for one parallel frontier session. The operator
# may run it alongside other Claude or Codex sessions; the in-progress label
# claim protocol in AGENTS.md prevents two sessions from working one ticket.
# Set a distinct CODEX_SESSION_ID (for example, s3) for each running session.
#
...
# conservative baseline for INTERACTIVE use. The loop passes permissive flags
# for this non-interactive invocation, so an unattended run never blocks on an
# approval prompt:
...
#
# Usage from the repository root: CODEX_SESSION_ID=s3 .codex/loop.sh
# If the operator assigns the ID separately, export it first:
#   export CODEX_SESSION_ID=s3
#   .codex/loop.sh
set -euo pipefail

if [[ -z "${CODEX_SESSION_ID:-}" ]]; then
  echo "Set CODEX_SESSION_ID to the supplied session id before starting Codex." >&2
  exit 2
fi
