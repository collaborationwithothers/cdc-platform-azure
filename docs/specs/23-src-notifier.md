...
queue-builder skips an unprocessable event and parks it, because the reconciler
heals `QueueState` afterwards so skipping costs nothing permanent. The notifier
cannot do that. Nothing heals a notification, so skipping drops one silently,
which is the exact failure ADR-008 pins the send-then-record ordering to
prevent. Doing both would leave the component contradicting itself.
...
| N5 | Metrics and the alert rule on `auto_park`, since a nonzero value means alarms are going unanswered. | unit | 3 files, 140 lines |
