...
```
 3x tenant DBs (Azure SQL, pooled GP_Gen5_2 at build scale)
...
- Tenant databases. Three Azure SQL databases share a build-scale General Purpose standard-series elastic pool with two vCores and 32 GB maximum data storage. Microsoft documents CDC support for elastic pools in every vCore service tier. The earlier Standard DTU pool remains UNVERIFIABLE and does not ship. Three CDC-enabled databases on two vCores exceed Microsoft's recommendation that their count should not exceed the pool's vCore count, so a later live load test measures capture latency and decides whether four vCores are required. Each database carries WorkflowTask (Id IDENTITY int, State, Version int, audit columns), Outbox, and a TenantInfo row holding the canonical tenantId (the attribution claim, see section 9). CDC is enabled on Outbox only; Change Tracking is enabled on WorkflowTask as the reconciler's committed-order change feed (ADR-009). WorkflowTask is never captured by CDC (ADR-001).
...
- DTU certainty versus vCore pooling: the documented CDC path won over the UNVERIFIABLE Standard DTU pool. Two vCores reduce build cost, but three CDC-enabled databases exceed Microsoft's pool-sizing recommendation. The live load test decides whether the measured latency justifies four vCores.
...
Production scale (400 tenants): modelled properly in a dedicated doc during build. The two-vCore build pool is not extrapolated to 400 databases. The model, not a guess, is the deliverable.
...
- Deploy: a manually bootstrapped state backend plus two Terraform layers. The
